...
matStats = torch.zeros(NUM_REQ, NUM_Q_HEAD, 2, dtype=torch.float, device=gpu)
...
def tma_load_split_attn_single(mat: torch.Tensor, tileS, tileO):
    assert tileS == 1
...
tO_split_single = TmaTensor(dae, matO_split_load_view)._build("load", 1, HEAD_DIM * split_q_tile, tma_load_split_attn_single, cord_load_split_attn)

def tma_reduce_o(mat: torch.Tensor, tileQ: int, tileD: int):
    assert tileD == HEAD_DIM
    assert tileQ == split_q_tile
    R, Q, D = mat.shape
    elsize = mat.element_size()
    glob_dims = [D, R * Q]
    glob_strides = [D * elsize]
    box_dims = [D, tileQ]
    rank = len(glob_dims)
    box_strides = [1] * rank
    return rank, runtime.build_tma_desc(
...
def cord_reduce_o(mat: torch.Tensor, rank: int):
    assert rank == 2
    def cfunc(*cords):
        assert len(cords) == 2, f"cords should be (req, q_ofst), but got {cords}"
        req, q_ofst = cords
        return [0, req * NUM_Q_HEAD + q_ofst]
    return cfunc

tO_reduce = TmaTensor(dae, matO_attn_Q_view)._build("reduce", split_q_tile, HEAD_DIM, tma_reduce_o, cord_reduce_o)

need_norm = False
need_rope = False

attn_bar = dae.new_bar(NUM_REQ * NUM_KV_HEAD * split_kv)
stats_bar = dae.new_bar(NUM_REQ * NUM_Q_HEAD // split_q_tile)
...
def sm_task(sm: int):
    insts = []
    qtile_workers = NUM_REQ * NUM_Q_HEAD // split_q_tile
    correction_workers = split_kv * NUM_REQ * NUM_Q_HEAD // split_q_tile
    ...
    if sm < qtile_workers:
        ...
        insts += [
            ATTN_SPLIT_GLOBAL_REDUCE(split_kv, split_q_tile, q_ofst, NUM_Q_HEAD),
            TmaLoad1D(matP[req, :split_kv]).bar(attn_bar),
            TmaStore1D(matStats[req, q_ofst:q_ofst + split_q_tile]).bar(stats_bar),
        ]

    if sm < correction_workers:
        req = sm // (split_kv * (NUM_Q_HEAD // split_q_tile))
        rem = sm % (split_kv * (NUM_Q_HEAD // split_q_tile))
        split_stage = rem // (NUM_Q_HEAD // split_q_tile)
        q_tile_idx = rem % (NUM_Q_HEAD // split_q_tile)
        q_ofst = q_tile_idx * split_q_tile
        insts += [
            ATTN_SPLIT_LOCAL_CORRECT(split_q_tile, NUM_Q_HEAD),
            TmaLoad1D(matP[req, split_stage, 0, q_ofst:q_ofst + split_q_tile]).bar(attn_bar),
            TmaLoad1D(matStats[req, q_ofst:q_ofst + split_q_tile]).bar(stats_bar),
            tO_split_single.cord(split_stage, req, q_ofst),
            tO_reduce.cord(req, q_ofst),
        ]
    return insts
