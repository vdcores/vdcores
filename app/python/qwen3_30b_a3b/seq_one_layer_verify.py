...
from dae.tma_utils import StaticCordAdapter, ToAttnVStoreCordAdapter, ToConvertedCordAdapter
...
q_proj = SchedGemv(Gemv_M64N8, MNK=(QW, N, HIDDEN), tmas=(dense_weight(weightg["loadQW"]), weightg["loadRMSLayer64"], weightg["storeQ"])).bar("store", sysg["bar_q_proj"]).place(64)
k_proj = SchedGemv(Gemv_M64N8, MNK=(KW, N, HIDDEN), tmas=(dense_weight(weightg["loadKW"]), weightg["loadRMSLayer64"], ToAttnVStoreCordAdapter(weightg["storeK"], 0))).bar("store", sysg["bar_qkv_attn"]).place(16, base_sm=64)
v_proj = SchedGemv(Gemv_M64N8, MNK=(VW, N, HIDDEN), tmas=(dense_weight(weightg["loadVW"]), weightg["loadRMSLayer64"], ToAttnVStoreCordAdapter(weightg["storeV"], 0))).bar("store", sysg["bar_qkv_attn"]).place(16, base_sm=80)
gqa = SchedAttentionDecoding(reqs=N, seq_len=1, KV_BLOCK_SIZE=KVBlockSize, NUM_KV_HEADS=NUM_KV_HEAD, matO=matO_attn_view, tmas=(weightg["loadQ"], weightg["loadK"], weightg["loadV"]), side_input=side_input, k_store=current_k_store, token_pos=0).bar("q", sysg["bar_q_proj"]).bar("k", sysg["bar_qkv_attn"]).bar("o", sysg["bar_attn_out"]).place(N * NUM_KV_HEAD)
...
post_attn_rms = SchedRMSShared(num_token=N, epsilon=eps, hidden_size=HIDDEN, tmas=(dense_weight(weightg["loadRMSPostAttnW"]), TmaLoad1D(matHidden), TmaStore1D(matRMSHidden))).bar("input", sysg["bar_out_mlp"]).bar("output", sysg["bar_post_attn_rms"]).place(rms_sms)
...
final_rms = SchedRMSShared(num_token=N, epsilon=eps, hidden_size=HIDDEN, tmas=(dense_weight(weightg["loadRMSInputWLoop"]), TmaLoad1D(matHidden), TmaStore1D(matRMSHidden))).bar("output", sysg["bar_final_rms"]).place(rms_sms)
...
argmax = SchedArgmax(num_token=N, logits_slice=logits_slice, num_slice=logits_epoch, AtomPartial=ARGMAX_PARTIAL_bf16_1152_50688_132, AtomReduce=ARGMAX_REDUCE_bf16_1152_132, matLogits=matLogits, matOutVal=matArgmaxVal, matOutIdx=matArgmaxIdx, matFinalOut=matTokens[:, 1]).bar("load", sysg["bar_logits"]).bar("val", sysg["bar_argmax_val"]).bar("idx", sysg["bar_argmax_idx"]).bar("final", sysg["bar_token_finish"]).place(full_sms)
...
late_bar_schedules = [q_proj, k_proj, v_proj, gqa, out_proj, post_attn_rms, router_proj, router_topk]

reg_gate, reg_up = 0, 1
for slot in range(TOP_K):
    gate_proj = SchedGemv(Gemv_M64N8, MNK=(MOE_INTERMEDIATE, N, HIDDEN), tmas=(expert_weight(weightg["loadExpertGateWs"]), weightg["loadRMSLayer64"], RegStore(reg_gate, matExpertAct[slot][:, :TileM64]))).place(12)
    up_proj = SchedGemv(Gemv_M64N8, MNK=(MOE_INTERMEDIATE, N, HIDDEN), tmas=(expert_weight(weightg["loadExpertUpWs"]), weightg["loadRMSLayer64"], RegStore(reg_up, matExpertAct[slot][:, :TileM64]))).place(12)
