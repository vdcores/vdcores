...
def build_tma_stacked_row(mat: torch.Tensor, tile_size: int, _: int):
    assert mat.dim() == 2, "stacked row TMA expects [rows, width]"
    row_size = mat.shape[-1]
    elsize = mat.element_size()
    chunk = 64 // elsize
    assert row_size % chunk == 0, "row size must align to 64 bytes"
    assert tile_size == row_size, "stacked row TMA only supports whole-row transfers"
    global_dims = [chunk, row_size // chunk, mat.shape[0]]
    global_strides = [chunk * elsize, row_size * elsize]
    box_dims = [chunk, tile_size // chunk, 1]
    box_strides = [1, 1, 1]
...
def cord_func_stacked_row(mat: torch.Tensor, rank: int):
    assert rank == 3, f"Expected rank-3 stacked row TMA, got {rank}"

    def cord_func(*cords):
        assert len(cords) == 2, f"stacked row cords should be (row, byte_offset), got {cords}"
        return [0, cords[1] // 32, cords[0]]
