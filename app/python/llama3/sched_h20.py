...
layerg.addBarrier('bar_gateup_0')
layerg.addBarrier('bar_gateup_1')
layerg.addBarrier('bar_silu_low')
layerg.addBarrier('bar_silu_tail')
...
def schedule_single_token(
  ...
  mlp_tiles = (74, 74, 76)
  mlp_0 = mlp_tiles[0] * TileM
  mlp_1 = mlp_tiles[1] * TileM
  mlp_2 = mlp_tiles[2] * TileM
  mlp_01 = mlp_0 + mlp_1

  gate_proj_0 = SchedGemv(Gemv_M64N8,
    MNK=(mlp_0, N, HIDDEN),
    tmas=(layerg['loadGate'], layerg['loadRMSLayer'], layerg['storeGateOut']),
  ).bar("load", layerg['bar_post_attn_rms'])
  up_proj_0 = SchedGemv(Gemv_M64N8,
    MNK=(mlp_0, N, HIDDEN),
    tmas=(layerg['loadUp'], layerg['loadRMSLayer'], layerg['storeInterm']),
  ).bar("load", layerg['bar_post_attn_rms'])
  gate_proj_0.bar("store", layerg['bar_gateup_0'])
  up_proj_0.bar("store", layerg['bar_gateup_0'])
  silu_0 = SchedSmemSiLUInterleavedK(
    num_token=N,
    gate_glob=matGateOut[:, :mlp_0],
    up_glob=matInterm[:, :mlp_0],
    out_glob=matSiLUOut[:, :mlp_0],
  ).bar("input", layerg['bar_gateup_0']).bar("output", layerg['bar_silu_low'])

  gate_proj_1 = SchedGemv(Gemv_M64N8,
    MNK=((mlp_0, mlp_1), N, HIDDEN),
    tmas=(layerg['loadGate'], layerg['loadRMSLayer'], layerg['storeGateOut']),
  ).bar("store", layerg['bar_gateup_1'])
  up_proj_1 = SchedGemv(Gemv_M64N8,
    MNK=((mlp_0, mlp_1), N, HIDDEN),
    tmas=(layerg['loadUp'], layerg['loadRMSLayer'], layerg['storeInterm']),
  ).bar("store", layerg['bar_gateup_1'])
  ...
  ).bar("input", layerg['bar_gateup_1']).bar("output", layerg['bar_silu_low'])
  ...
  silu_fused = SchedRegSiLUFused(
    ...
    base_offset=mlp_01,
    stride=TileM,
  ).bar("output", layerg['bar_silu_tail'])
  ...
  down_proj_high = SchedGemv(Gemv_M64N8,
    MNK=(HIDDEN, N, (8192, INTERMIDIATE - 8192)),
    tmas=(layerg['loadDown'], layerg['loadSiluLayer'], layerg['reduceHiddenLayer'])
  ).bar("load", layerg['bar_silu_tail']).bar("store", layerg['bar_layer'])
  down_proj_low.bar("load", layerg['bar_silu_low'])
  ...
  if h20_mode:
    gate_proj_0 = gate_proj_0.place(mlp_tiles[0])
    up_proj_0 = up_proj_0.place(mlp_tiles[0])
    silu_0 = silu_0.place(8, base_sm=64)
    gate_proj_1 = gate_proj_1.place(mlp_tiles[1])
    up_proj_1 = up_proj_1.place(mlp_tiles[1])
    silu_1 = silu_1.place(8, base_sm=64)
    gate_proj_fused = gate_proj_fused.place(mlp_tiles[2])
    up_proj_fused = up_proj_fused.place(mlp_tiles[2])
    silu_fused = silu_fused.place(mlp_tiles[2])
    down_proj_low = down_proj_low.place(64)
    down_proj_high = down_proj_high.place(64)
  else:
    gate_proj_0 = gate_proj_0.place(mlp_tiles[0])
    up_proj_0 = up_proj_0.place(mlp_tiles[0], base_sm=full_sms - mlp_tiles[0])
    silu_0 = silu_0.place(4, base_sm=128)
    gate_proj_1 = gate_proj_1.place(mlp_tiles[1])
    up_proj_1 = up_proj_1.place(mlp_tiles[1], base_sm=full_sms - mlp_tiles[1])
    silu_1 = silu_1.place(4, base_sm=128)
    gate_proj_fused = gate_proj_fused.place(mlp_tiles[2])
    up_proj_fused = up_proj_fused.place(mlp_tiles[2])
    silu_fused = silu_fused.place(mlp_tiles[2])
...
def run_correctness_check():
  ...
  final_checks = [
    check_tensor_threshold("gate_proj_low", layer['gate_proj'][0, decode_index, :9472], matGateOut[0, :9472], 5.0),
    check_tensor_threshold("up_proj_low", layer['up_proj'][0, decode_index, :9472], matInterm[0, :9472], 5.0),
