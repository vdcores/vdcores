...
once. A communication-specialized VDCores block executes one macro
`PoolSliceExchange`: it publishes route counts, packs route-major delivery
rows, dynamically gathers ready source batches into contiguous reader storage,
releases reader blocks, returns their output, and source-scatters it.
...
NVSHMEM_IBGDA_NUM_RC_PER_PE=1 NVSHMEM_IBGDA_RC_MAP_BY=cta \
...
Each source publishes one 64-byte descriptor to every target, including a
zero-row descriptor. One source-indexed signal word carries metadata, data,
and return phases for a monotonic sequence, so readers retire from the shared
sender set without a special epoch-end message. Rows are fixed-width,
16-byte-aligned, at least 1 KiB, and use fixed-capacity symmetric buffers.
The default `--pack-warps 0` selects four pack warps normally and six for a
large two-PE payload; explicit values are intended for tuning sweeps.
