...
for slot in range(TOP_K):
    ...
    sysg.addBarrier(f"bar_down{slot}")
...
weightg.addTma("loadRMSInputWLoop", [matRMSInputWLoop], lambda t: t.indexed("layer")._build("load", HIDDEN, 1, build_tma_stacked_row, cord_func_stacked_row))
...
q_proj = SchedGemv(Gemv_M64N8, MNK=(QW, N, HIDDEN), tmas=(dense_weight(weightg["loadQW"]), weightg["loadRMSLayer64"], weightg["storeQ"])).bar("load", sysg["bar_pre_attn_rms"]).bar("store", sysg["bar_q_proj"]).place(64)
k_proj = SchedGemv(Gemv_M64N8, MNK=(KW, N, HIDDEN), tmas=(dense_weight(weightg["loadKW"]), weightg["loadRMSLayer64"], ToAttnVStoreCordAdapter(weightg["storeK"], token_pos))).bar("load", sysg["bar_pre_attn_rms"]).bar("store", sysg["bar_qkv_attn"]).place(16, base_sm=64)
v_proj = SchedGemv(Gemv_M64N8, MNK=(VW, N, HIDDEN), tmas=(dense_weight(weightg["loadVW"]), weightg["loadRMSLayer64"], ToAttnVStoreCordAdapter(weightg["storeV"], token_pos))).bar("load", sysg["bar_pre_attn_rms"]).bar("store", sysg["bar_qkv_attn"]).place(16, base_sm=80)
gqa = SchedAttentionDecoding(reqs=N, seq_len=token_pos + 1, KV_BLOCK_SIZE=KVBlockSize, NUM_KV_HEADS=NUM_KV_HEAD, matO=matO_attn_view, tmas=(weightg["loadQ"], weightg["loadK"], weightg["loadV"]), side_input=side_input, k_store=current_k_store, token_pos=token_pos).bar("q", sysg["bar_q_proj"]).bar("k", sysg["bar_qkv_attn"]).bar("o", sysg["bar_attn_out"]).place(N * NUM_KV_HEAD)
out_proj = SchedGemv(Gemv_M64N8, MNK=(HIDDEN, N, QW), tmas=(dense_weight(weightg["loadOutWs"]), weightg["loadAttnOLayer"], weightg["reduceHiddenLayer"])).bar("load", sysg["bar_attn_out"]).bar("store", sysg["bar_out_mlp"]).place(64)
post_attn_rms = SchedRMSShared(num_token=N, epsilon=eps, hidden_size=HIDDEN, tmas=(dense_weight(weightg["loadRMSPostAttnW"]), loadHidden1D, storeRMSHidden1D)).bar("input", sysg["bar_out_mlp"]).bar("output", sysg["bar_post_attn_rms"]).place(rms_sms)
router_proj = SchedGemv(Gemv_M64N8, MNK=(128, N, HIDDEN), tmas=(dense_weight(weightg["loadRouterWs"]), weightg["loadRMSLayer64"], weightg["storeRouterLogits"])).bar("load", sysg["bar_post_attn_rms"]).bar("store", sysg["bar_router"]).place(2)
router_topk = SchedRouterTopK(num_token=N, logits_tma=StaticCordAdapter(TmaLoad1D(matRouterLogits)), weight_tma=StaticCordAdapter(TmaStore1D(matRouterTopKWeight)), idx_addr=StaticCordAdapter(RawAddress(matRouterTopKIdx, 24))).bar("input", sysg["bar_router"]).bar("output", sysg["bar_router_topk"]).place(1, base_sm=128)
final_rms = SchedRMSShared(num_token=N, epsilon=eps, hidden_size=HIDDEN, tmas=(dense_weight(weightg["loadRMSInputWLoop"]), loadHidden1D, storeRMSHidden1D)).bar("output", sysg["bar_final_rms"]).place(rms_sms)
...
argmax = SchedArgmax(num_token=N, logits_slice=logits_slice, num_slice=logits_epoch, AtomPartial=ARGMAX_PARTIAL_bf16_1152_50688_132, AtomReduce=ARGMAX_REDUCE_bf16_1152_132, matLogits=matLogits, matOutVal=matArgmaxVal, matOutIdx=matArgmaxIdx, matFinalOut=matTokens[:, token_offset + 1]).bar("load", sysg["bar_logits"]).bar("val", sysg["bar_argmax_val"]).bar("idx", sysg["bar_argmax_idx"]).bar("final", sysg["bar_token_finish"]).place(full_sms)

ops = [
    embed_rms, copy_hidden, IssueBarrier(sysg["bar_pre_attn_rms"]),
    SetLayerIndex(0),
    q_proj, IssueBarrier(sysg["bar_q_proj"]),
    k_proj, v_proj, IssueBarrier(sysg["bar_qkv_attn"]),
    gqa, IssueBarrier(sysg["bar_attn_out"]),
    out_proj, IssueBarrier(sysg["bar_out_mlp"]),
    post_attn_rms, IssueBarrier(sysg["bar_post_attn_rms"]),
    router_proj, IssueBarrier(sysg["bar_router"]),
    router_topk, IssueBarrier(sysg["bar_router_topk"]),
]
late_bar_schedules = [embed_rms, copy_hidden, q_proj, k_proj, v_proj, gqa, out_proj, post_attn_rms, router_proj, router_topk]

reg_gate, reg_up = 0, 1
for slot in range(TOP_K):
    gate_proj = SchedGemv(Gemv_M64N8, MNK=(MOE_INTERMEDIATE, N, HIDDEN), tmas=(expert_weight(weightg["loadExpertGateWs"]), weightg["loadRMSLayer64"], RegStore(reg_gate, matExpertAct[slot][:, :TileM64]))).bar("load", sysg["bar_post_attn_rms"]).place(12)
    up_proj = SchedGemv(Gemv_M64N8, MNK=(MOE_INTERMEDIATE, N, HIDDEN), tmas=(expert_weight(weightg["loadExpertUpWs"]), weightg["loadRMSLayer64"], RegStore(reg_up, matExpertAct[slot][:, :TileM64]))).bar("load", sysg["bar_post_attn_rms"]).place(12)
    silu = SchedRegSiLUFused(num_token=N, store_tma=weightg[f"storeExpertAct{slot}"], reg_gate=reg_gate, reg_up=reg_up, base_offset=0, stride=TileM64).bar("output", sysg[f"bar_scale{slot}"]).place(12)
    down_proj = SchedGemvMmaScale(Gemv_M64N8_MMA_SCALE, MNK=(HIDDEN, N, MOE_INTERMEDIATE), tmas=(expert_weight(weightg["loadExpertDownWs"]), weightg[f"loadExpertAct{slot}"], StaticCordAdapter(TmaLoad1D(matRouterTopKWeight[slot], bytes=64)), weightg["reduceHiddenLayer"])).bar("load", sysg[f"bar_scale{slot}"]).bar("store", sysg[f"bar_down{slot}"]).place(32)
    ops.extend([LoadExpertIndex(matRouterTopKIdx, slot), gate_proj, up_proj, silu, IssueBarrier(sysg[f"bar_scale{slot}"]), down_proj, IssueBarrier(sysg[f"bar_down{slot}"])])
    late_bar_schedules.extend([silu, down_proj])

ops.extend([final_rms, IssueBarrier(sysg["bar_final_rms"]), logits_proj, argmax, IssueBarrier(sysg["bar_token_finish"])])
late_bar_schedules.extend([final_rms, logits_proj, argmax])
