...
def calc_split_meta(num_sms, num_q_head, num_kv_head, head_dim, num_kv_block, split_o_load_limit_bytes: int = 16 * 1024):
    ...
    split_kv = infer_split_kv(num_sms, num_kv_head, num_kv_block, 128)
    split_q_tile = max(1, (num_q_head + num_sms - 1) // num_sms)
    splits_per_post_load = min(
        max(1, split_o_load_limit_bytes // (split_q_tile * head_dim * 2)),
        split_kv,
    )
    return split_kv, split_q_tile, splits_per_post_load
