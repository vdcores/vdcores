...
class Launcher:
    ...
    def bench(self, iterations : int = 100,
        ...
        for i in range(iterations):
            print(f"[bench] iteration {i + 1}/{iterations} start", flush=True)
            self.launch()
            print(f"[bench] iteration {i + 1}/{iterations} done", flush=True)

            # fetch profile data
            profile_data = self.profile[:,0:2].cpu().numpy()
            iter_duration = profile_data[:,1] - profile_data[:,0]
            duration_ns += iter_duration
        ...
        avg_duration_per_sm = duration_ns.double() / iterations
        mean_duration_ns = avg_duration_per_sm.mean().item()
        for sm_id, dur in enumerate(avg_duration_per_sm):
            print(f"SM {sm_id:3d} | Avg Duration: {dur:10.2f} ns")
        ...
        print(f"Average duration (ns): {mean_duration_ns:.2f}")
        ...
        if total_bytes is not None:
            bandwidth = total_bytes / (mean_duration_ns / 1e9) / (1024 **3) # GB/s
            print(f"Effective Bandwidth (GB/s): {bandwidth:.2f}")
        if total_flops is not None:
            gflops = total_flops / mean_duration_ns / 1e6
