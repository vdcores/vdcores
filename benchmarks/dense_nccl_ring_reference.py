"""Dense two-all-reduce NCCL ring surrogate shared by pool benchmarks.

This module intentionally remains outside the VDCores runtime and application
source trees. It is not NVIDIA NCCL EP: dispatch uses an expert-major dense
all-reduce and return uses a token-major dense all-reduce. Pool timing uses
internal VDCores profile events; this surrogate uses CUDA events and MPI
rank-max reduction.
"""
