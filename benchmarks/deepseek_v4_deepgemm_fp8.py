...
def main() -> None:
    ...
    activation_ref, activation_scale_ref = per_token_cast_to_fp8(
        activation_source, use_ue8m0=True, gran_k=128
    )
    activation, activation_scale = (
        per_token_group_quant_fp8_packed_for_deepgemm(
            activation_source, group_size=128, use_ue8m0=True
        )
    )
    ...
    weight, weight_scale_ref = per_block_cast_to_fp8(
        weight_source, use_ue8m0=True, gran_k=128
    )
    ...
    activation_dequant = (
        activation_ref.float()
        * activation_scale_ref.float().repeat_interleave(128, dim=1)
    )
    weight_dequant = (
        weight.float()
        * weight_scale_ref.float().repeat_interleave(128, dim=0).repeat_interleave(
            128, dim=1
        )
    )
    reference = (activation_dequant @ weight_dequant.t()).to(torch.bfloat16)
    ...
    print(
        ...
        f"shape={args.m}x1x{args.k} inner={args.inner} "
