...
struct PipelineBarriers {
  cuda::barrier<cuda::thread_scope_block> q_fill;
  cuda::barrier<cuda::thread_scope_block> q_drain;
...
__device__ __forceinline__ void init_pipeline_barriers(PipelineBarriers& barriers) {
  if (threadIdx.x == 0) {
    init(&barriers.q_fill, kComputeThreads + kProducerThreads);
    init(&barriers.q_drain, kComputeThreads + kProducerThreads);
