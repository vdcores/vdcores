...
void dae_ffn_down_direct_kernel(
  ...
  void *smem_base = dae_ffn_align_to(dynamic_shared, 1024);
  __shared__ alignas(16) uint32_t tmem_base_ptr;
  constexpr int kDownTmemColumns = 64;
  cute::TMEM::Allocator1Sm tmem_allocator{};
  if (threadIdx.x / numThreadsPerWarp == 0) {
  ...
  __syncthreads();
  if (threadIdx.x / numThreadsPerWarp == 0) {
    // The allocation permit serializes subsequent allocators on this SM; it
    // does not guard use of the allocated columns. Relinquish it immediately
    // so a second resident down CTA can obtain a disjoint 64-column slice.
    tmem_allocator.release_allocation_lock();
  }
