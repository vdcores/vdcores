...
// Use the system variants only when an NIC or another GPU consumes the data.
...
// Returns the previous value so the last contributor can publish the next
// named dependency. Acq-rel chains payload stores from every contributing
// warp through the counter's modification order without a general fence.
static __device__ __forceinline__ uint64_t dae_atomic_fetch_add_acq_rel_gpu(
    uint64_t* address, uint64_t value) {
  uint64_t previous;
  asm volatile(
...
static __device__ __forceinline__ void dae_atomic_add_release_sys(
    uint64_t* address, uint64_t value) {
  asm volatile(
...
static __device__ __forceinline__ uint64_t dae_atomic_load_acquire_sys(
    const uint64_t* address) {
  uint64_t value;
  asm volatile(
