...
template<int M, int N, int K, typename M2C_Type, typename C2M_Type>
__device__ __forceinline__ void task_gemv_mma_scale(const int nTiles, void *base, M2C_Type& m2c, C2M_Type& c2m) {
    using namespace cute;
    ...
    using data_t = typename AtomTrait::ValTypeA;
    using accum_t = typename AtomTrait::ValTypeC;
    using scale_vec_t = typename F16Traits<data_t>::vec2_t;
    ...
    auto tiled_mma = make_tiled_mma(
        MMA_Atom<Atom>{},
        make_layout(make_shape(Int<M / MMA_M>{}, Int<1>{}, Int<1>{})),
        make_tile(Int<M>{}, Int<MMA_N>{}, Int<K>{})
    );
    auto thr_mma  = tiled_mma.get_slice(tid);
    ...
    int scale_slot = m2c.template pop<0>();
    data_t *scale_ptr = (data_t *)get_slot_address(base, extract(scale_slot));
    ...
    for (int i = 0; i < nTiles; i++) {
        ...
        for (int idx = tid; idx < N * K; idx += numThreads) {
            int n = idx / K;
            int k = idx % K;
            sB(n, k) = data_t(float(sB(n, k)) * float(scale_ptr[n]));
        }
        __sync_compute_group(numThreads);
        copy(thr_mma.partition_B(sB), frag_B);
