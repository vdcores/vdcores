...
- Queue ordering is a cross-task ABI, not an attention-only detail: see `agents/knowledge/task-queue-semantics.md` before changing any `include/task/*.cuh` kernel or the corresponding shared-memory instruction order in `python/dae/schedule.py`.
