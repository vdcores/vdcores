...
- `python/dae/compiled_mode.py`: compiled-mode spec exporter and runtime-payload analyzer. For the current standalone subset it encodes static compute args, static memory fields, and `RepeatM` deltas into the AOT manifest, while collecting only the truly live `uint64` values still needed at launch time.
...
- The compiled standalone subset no longer streams full `CInst` / `MInst` buffers at launch time: the generated compiled program now synthesizes spec-encoded compute/memory literals in CUDA and consumes only a compact per-SM live-value buffer, which is currently used just for direct 1D load/store base pointers.
