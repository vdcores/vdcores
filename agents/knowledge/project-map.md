...
- `python/dae/compiled_mode.py`: compiled-mode spec exporter and runtime-payload analyzer. For the current standalone subset it encodes structural compute/memory flow and `RepeatM` deltas into the AOT manifest, while moving per-SM address / coord seed values into a compact `uint64` payload so structurally identical SM streams can share one generated program. It now also preserves full memory opcode bits and full `num_slots` packing for compiled programs, which is required for barrier-tagged TMA ops plus pseudo-memory carriers such as `RegLoad`, `RegStore`, and `RawAddress`. Simple non-repeat `RegStore`/`RegLoad` pairs are now recognized at spec time and tagged so the generated LDU code can use a dedicated one-shot local instead of a generic register table.
...
- `tools/generate_compiled_program.py`: build-time helper that consumes `dae_compiled_program.vdcore.json` or `DAE_COMPILED_SPEC_FILE` and emits `build/generated/dae/compiled_program.inc` for the compiled-kernel path. For simple paired `RegStore`/`RegLoad` usage it now emits dedicated per-program locals such as `__reg_pair_0` instead of reconstructing those ops through a generic `regFile[32]` array. Double-port compiled programs now also emit separate LDU entry points (`dae_compiled_ld0_execute` and `dae_compiled_ld1_execute`) so the port split is compile-time rather than a runtime `port_id` branch inside generated code.
