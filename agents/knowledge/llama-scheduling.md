...
- Multi-block decode relies on consecutive token/block `OP_REPEAT` seeds remaining independent. On 2026-04-21, applying active repeat address accumulation to `OP_REPEAT` itself made the full-block body write KV rows at `128 + 2*c` and leave alternating holes after the first block. `include/dae/pipeline/allocwarp.cuh` now skips address shifting for `OP_REPEAT`, while ordinary allocating repeat consumers still receive the accumulated token/block offset.
