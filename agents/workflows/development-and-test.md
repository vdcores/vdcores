...
- As a file-based alternative, put one operator symbol per line in a repo-root `dae_compute_ops.txt` file, or point `DAE_COMPUTE_OPS_FILE` at another file; the build prints which source it used.
