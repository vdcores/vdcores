...
- A clean `make pyext` now generates `build/generated/dae/selected_compute_ops.inc`, `build/generated/dae/compute_opcode_order.inc`, and `build/generated/dae/dynamic_compute_handlers.inc` in the Makefile before compiling `runtime.o`; `setup.py` consumes those generated includes but does not create them.
- For family-backed compute instructions such as the canonical GEMV strings, `-w` can write the family string before the matching runtime opcode exists. The launcher now defers the missing-opcode failure to `launch()`/`bench()`, so the intended loop is: run `-w`, rebuild `dae.runtime`, then rerun with `-l` or `-b`.
